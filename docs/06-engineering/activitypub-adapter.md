...
Federation is wiring. This file describes that wiring; the *meaning* lives in [the federation section](../05-federation/README.md), and the load-bearing idea is [ActivityPub as infrastructure](../05-federation/activitypub-as-infrastructure.md): **the protocol is like email — the person never sees it.** No federation code exists in the repo yet; this is the design we build toward, isolated in `packages/federation` (see [folder structure](folder-structure.md)).
