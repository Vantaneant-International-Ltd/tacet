...
export async function fetchCollectionItems(collectionUrl: string, limit: number, maxPages = 3): Promise<unknown[]> {
  const out: unknown[] = [];
  let node: unknown = await fetchAp(collectionUrl);
