...
import { AtprotoAdapter } from "./atproto/adapter";
...
// it is read live per request so its existing behaviour is never disturbed.
export function collectorAdapters(_cfg: SourcesConfig = {}): SourceAdapter[] {
  return [new FeedsAdapter(), new AtprotoAdapter(), new NostrAdapter()];
}
